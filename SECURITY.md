...
If you discover a security vulnerability in hydradb-claude-code, please report it responsibly. **Do not open a public GitHub issue for security vulnerabilities.**
...
- The affected version(s) of hydradb-claude-code.
...
This security policy covers the hydradb-claude-code repository, including:

- All plugin source code, hooks, and skill definitions.
...
- Third-party dependencies (report those to the respective maintainers, but let us know if a dependency vulnerability affects hydradb-claude-code).
...
When contributing to hydradb-claude-code, follow these security practices:

- Never commit API keys, tokens, passwords, or other credentials to the repository.
- Use environment variables or the `.hydradb-plugin.json` configuration file for sensitive settings (see `.hydradb-plugin.json.example`).
